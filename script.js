...
(function() {
    ...
    const MAX_TRAIL = 120; // OPTIMAL: 120 titik (dari 600)
    const GRAVITY_DEFAULT = 0.25;

    // ============================================================
    // 6. VARIABEL
    // ============================================================
    let v0 = 14, sudut = 45, g = GRAVITY_DEFAULT, dt = 1;
    ...
    function resetSimulasi() {
        ...
        updateDataPanel();
        draw();
        stopAnimation(); // Hentikan animasi saat idle
    }
    ...
    function updateFisika() {
        ...
        // Simpan trail dengan batas MAX_TRAIL
        ...
        if (y >= GROUND_Y) {
            ...
            stopAnimation(); // Hentikan animasi saat selesai
        }
    ...
    function tampilkanHasilPrediksi() {
        ...
        const jawaban = jawabanDipilih.dataset.jawaban;
        const sudutSekarang = parseInt(sudutSlider.value);
        let benar = false;
        if (sudutSekarang <= 45 && jawaban === 'bertambah') benar = true;
        else if (sudutSekarang > 45 && jawaban === 'berkurang') benar = true;
        else if (sudutSekarang === 45 && jawaban === 'tetap') benar = true;

        hasilPrediksi.style.display = 'block';
        hasilPrediksi.className = 'hasil-prediksi ' + (benar ? 'benar' : 'salah');
        hasilPrediksi.innerHTML = benar ?
            '✅ <strong>Benar!</strong> ' + (sudutSekarang <= 45 ? 'Sudut yang lebih besar menghasilkan jarak yang lebih jauh (hingga 45°).' : 'Sudut yang lebih besar dari 45° mengurangi jarak horizontal.') :
            '❌ <strong>Kurang tepat.</strong> ' + (sudutSekarang <= 45 ? 'Coba ingat: sudut yang lebih besar (hingga 45°) menghasilkan jarak yang lebih jauh.' : 'Sudut yang lebih besar dari 45° justru mengurangi jarak horizontal.');
    }

    // ============================================================
    // 16. GAMBAR MERIAM (RINGAN)
    // ============================================================
    ...
    function animate() {
        if (running && !paused) {
            ...
            // Jika tidak running (idle/selesai), hentikan loop
            stopAnimation();
            draw();
        }
    }
    ...
    function handleCanvasClick(event) {
        ...
        if (titikTerdekat) {
            const vTotal = Math.sqrt(titikTerdekat.vx * titi
